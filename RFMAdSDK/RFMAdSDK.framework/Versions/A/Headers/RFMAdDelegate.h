...
/**
 * Sent just before reporting to Ad Forensics server begins, when the default or customized
 * Ad Forensics gesture recognizer is triggered.
 *
 * @warning We strongly recommend that you handle this delegate callback
 * to stop requesting new ads with the RFMAdview instance when you receive this callback. Otherwise
 * there could be unpredicted behaviors if the RFMAdview is refreshed while Ad Forensics reporting is
 * in progress.
 *
 * @param adView The instance of RFMAdView for which this callback has been triggered
 */
...
/**
 * Sent just after reporting to Ad Forensics server finishes, when reporting connection is
 * closed and Ad Forensics progress dialog is dismissed.
 *
 * Note: If you implement adForensicsReportingWillBegin, we strongly
 * recommend you to implement the following two delegate callbacks and resume requesting new ads with
 * the RFMAdview instance.
 *
 * @param adView The instance of RFMAdView for which this callback has been triggered
 */
